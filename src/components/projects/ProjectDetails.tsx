...
export function ProjectDetails({ project, detail }: ProjectDetailsProps) {
    ...
    return (
        <>
            <div className="relative w-full aspect-video border-b border-[var(--color-border)]">
                <img src={images[imgIndex]} alt={`${project.name} screenshot ${imgIndex + 1}`} className="w-full h-full object-cover object-top rounded-t-2xl" />
                {images.length > 1 && (
                    <>
                        <button onClick={prevImage} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/60 rounded-full p-1 text-[var(--color-text)] hover:bg-black/80 transition-colors">
                            <ChevronLeft size={20} />
                        </button>
                        <button onClick={nextImage} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/60 rounded-full p-1 text-[var(--color-text)] hover:bg-black/80 transition-colors">
